...
public static class WorldWindow {
						...
						PopupManager.Add(new AcronymPopup((acronym) => {
							PopupManager.Add(new ConfirmPopup($"Change Acronym to {acronym}?").SetButtons("Yes", "No").Okay(() => {
								PopupManager.Add(new ConfirmPopup($"Copy or Replace existing files?").SetButtons("<s:1>Replace", "Copy").Okay(() => {
									ChangeAcronym(acronym, true);
								}).Cancel(() => {
									ChangeAcronym(acronym, false);
	...
	private static void ChangeAcronym(string newAcronym, bool replaceExistingFiles) {
		string oldAcronym = region.acronym;
		...
		string newExportPath = region.roomsPath[..(region.roomsPath.IndexOfReverse('\\') + 1)] + newAcronym;
		...
		string[] roomsOriginalFiles = Directory.Exists(region.roomsPath) ? Directory.GetFileSystemEntries(region.roomsPath) : [];
		Logger.Info("originalFiles:");
		foreach (string file in roomsOriginalFiles) {
			Logger.Info(" - " + file[(region.roomsPath.Length + 1)..]);
		}
		...
		// - go through region.rooms, create the conversions dictionary
		...
		foreach (Room room in region.rooms) {
			...
			if (room is not OffscreenRoom) {
				...
				if (room.path[(roomFinalBackslash + 1)..].Equals(oldName + ".txt", StringComparison.InvariantCultureIgnoreCase)) {
					...
					if (roomPath.Equals(region.roomsPath, StringComparison.InvariantCultureIgnoreCase))
		...
		foreach (string nonCheckedFile in nonCheckedRoomsFiles) {
			...
			if (Directory.Exists(nonCheckedFile)) {
				roomsSubDirectoriesToTransfer.Add((nonCheckedFile, nonCheckedFile.Replace(region.roomsPath, newRoomsPath, StringComparison.InvariantCultureIgnoreCase)));
		...
		string[] worldOriginalFiles = Directory.Exists(region.exportPath) ? Directory.GetFileSystemEntries(region.exportPath) : [];
		...
		foreach (string file in worldOriginalFiles) {
			...
			if (file.Equals(WorldWindow.region.exportPath + '\\' + "properties.txt", StringComparison.InvariantCultureIgnoreCase)) {
				...
				if (Directory.Exists(file)) {
					worldSubDirectoriesToTransfer.Add((file, file.Replace(region.exportPath, newExportPath)));
					Logger.Info($" <- add directory");
				}
				else {
					worldFilesToTransfer.Add((file, file.Replace(region.exportPath, newExportPath), -1));
					Logger.Info($" <- add -1");
				}
			}
		}
		...
		if (replaceExistingFiles) {
			...
			Directory.GetFileSystemEntries(region.roomsPath, "", searchOption: SearchOption.AllDirectories).ForEach(f => FloodForge.Backup.File(f));
			Directory.GetFileSystemEntries(region.exportPath, "", searchOption: SearchOption.AllDirectories).ForEach(f => FloodForge.Backup.File(f));
			Logger.Info($"FINISHED BACKING UP FILES!");
			Logger.Info($"! DELETING ORIGINAL FILES");
			Directory.Delete(region.exportPath, true);
			Directory.Delete(region.roomsPath, true);
			Logger.Info($"! FINISHED DELETING ORIGINAL FILES");
		}

		Logger.Info($"Setting new paths");
		region.exportPath = newExportPath;
		region.roomsPath = newRoomsPath;
		region.acronym = newAcronym;
